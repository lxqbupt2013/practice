...
func showImportantWords(name string) {
	fin, _ := os.Open("./diaries/" + name)
	...
	for text := range count {
		// 忽视一些不重要的词
		if isIgnoredWord(text) {
			continue
		}
		if count[text] > most {
			most = count[text]
			words = text
		} else if count[text] == most {
			words += " " + text
		}
	}
	fmt.Println("the most important words in", name, ":", words)
}

func main() {
	// 获取文件列表
	texts, _ := ioutil.ReadDir("./diaries")
	for _, v := range texts {
		showImportantWords(v.Name())
	}
}
